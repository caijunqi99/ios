...
@property (nonatomic ,strong) NSMutableArray<BMKPointAnnotation*> *pointAnnotationArry;
...
- (instancetype)initWihtBlcok:(AddOverBlcok)blcok{
    ...
    if (self) {
        ...
        self.pointAnnotationArry = [NSMutableArray array];
    }
    return self;
}
...
- (UIView *)searchBtn{
    if (!_searchBtn) {
        ...
        lab.text = @"搜索客户名称";
        lab.font = FontRegularWithSize(16);
...
- (UIButton *)reLocationBtn{
    if (!_reLocationBtn) {
        ...
        [_reLocationBtn setImage:[UIImage imageNamed:@"bluePoint"] forState:UIControlStateNormal];//reLocation
...
- (void)viewDidLoad {
    ...
    [self.mapView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(self.view.height/3.0);
    }];
...
- (void)userLocation{
    ...
    [[BaiDuLocationTool sharedInstance] getAddressDetail:^(AddressModel *addressModel) {
        ...
        [[PoiSearchHelpTool sharedInstance] startGeoSearchWithLocation:weakSelf.mapView.centerCoordinate block:^(id  _Nonnull object) {
            ...
            BMKReverseGeoCodeSearchResult *result = (BMKReverseGeoCodeSearchResult*)object;
            if ([result.poiList count]) {
                self->selectIndex = 0;
                [weakSelf.mapView removeAnnotations:weakSelf.pointAnnotationArry];
                [weakSelf.pointAnnotationArry removeAllObjects];
                ...
                for (BMKPoiInfo *info in result.poiList) {
                    BMKPointAnnotation * point = [[BMKPointAnnotation alloc]init];
                    point.coordinate = info.pt;
                    point.title = info.address;
                    [weakSelf.pointAnnotationArry addObject:point];
                    
                }
                
                [weakSelf.mapView addAnnotations:weakSelf.pointAnnotationArry];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.backBlock(self.dataArry[indexPath.row].name);
    [self.navigationController popViewControllerAnimated:YES];
    return;
    if (indexPath.row == selectIndex) {
        return;
    ...
    //开启反地理编码查询周边
    [[PoiSearchHelpTool sharedInstance] startGeoSearchWithLocation:self.dataArry[indexPath.row].pt block:^(id  _Nonnull object) {
...
//    GPDebugLog(@"%f",mapView.centerCoordinate.latitude);
