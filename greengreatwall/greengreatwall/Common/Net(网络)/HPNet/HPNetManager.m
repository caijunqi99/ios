                        ...
                        progressBlock:(HPDownloadProgressBlock)progressBlock
{
    ...
    if (isNeedCache && responseCacheData != nil)
    {
        ...
        GPDebugLog(@"取用缓存数据结果： *** %@", responseCacheData);
    ...
    else if (type == HPHttpRequestTypePost)
    {
        sessionTask = [HPNetManagerShare.sessionManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            GPDebugLog(@"上传进度--%lld, 总进度---%lld",uploadProgress.completedUnitCount,uploadProgress.totalUnitCount);
                                 ...
                                 progressBlock:(HPUploadProgressBlock)progressBlock
{
    ...
    sessionTask = [HPNetManagerShare.sessionManager POST:URLString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([obj isKindOfClass:[PHAsset class]])
            {
                ...
                [imageManager requestImageForAsset:asset targetSize:CGSizeMake(asset.pixelWidth , asset.pixelHeight) contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                    ...
                    GPDebugLog(@" width:%f height:%f",result.size.width,result.size.height);
        ...
        GPDebugLog(@"上传图片成功 = %@",responseObject);
        if (successBlock)
        {
            successBlock(responseObject);
        }
        ...
        switch ([avAssetExport status]) {
            case AVAssetExportSessionStatusCompleted:
            {
                [HPNetManagerShare.sessionManager POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                    ...
                    GPDebugLog(@"上传视频成功 = %@",responseObject);
                    if (successBlock)
                    {
                        successBlock(responseObject);
                    }
                } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
                    GPDebugLog(@"上传视频失败 = %@", error);
                    if (failureBlock)
                    {
                        failureBlock(error);
                    }
                                  ...
                                  progressBlock:(HPDownloadProgressBlock)progressBlock
{
    ...
    sessionTask = [HPNetManagerShare.sessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        GPDebugLog(@"下载进度：%.2lld%%",100 * downloadProgress.completedUnitCount/downloadProgress.totalUnitCount);
        /*! 回到主线程刷新UI */
        dispatch_async(dispatch_get_main_queue(), ^{
                          ...
                          index:(NSUInteger)index
{
    ...
    {   // 图片数据不为空才传递 fileName
        ...
        [formData appendPartWithFileData:imageData
                                    name:fileNames ? [NSString stringWithFormat:@"%@",fileNames[index]] : imageFileName
                                fileName:fileNames ? [NSString stringWithFormat:@"%@.%@",fileNames[index],imageType?:@"jpg"] : imageFileName
                                mimeType:[NSString stringWithFormat:@"image/%@",imageType ?: @"jpg"]];
        
        GPDebugLog(@"上传图片 %lu-%@ 成功", (unsigned long)index,fileNames ? [NSString stringWithFormat:@"%@",fileNames[index]] : imageFileName);
    }
}
...
+ (NSData *)compressOriginalImage:(UIImage *)image toMaxDataSizeKBytes:(CGFloat)size{
    ...
    double imageDatalength = [imageData length];
    if (imageDatalength>size)
    {
        imageData = UIImageJPEGRepresentation(image,size/imageDatalength);
    }
    ...
    GPDebugLog(@"data.length:----%lu",(unsigned long)data.length);
    
    if (data.length < size) return data;
    
    while (data.length>1024*1024*1024) {//1M以及以上
        data=UIImageJPEGRepresentation(newImage, 0.01);
        newImage = [UIImage imageWithData:data];
    }
    
    while (data.length>1024*1024) {//1M以及以上
        data=UIImageJPEGRepresentation(newImage, 0.1);
        newImage = [UIImage imageWithData:data];
    }
    
    while(data.length>512*1024) {//0.5M-1M
        data=UIImageJPEGRepresentation(newImage, 0.5);
        newImage = [UIImage imageWithData:data];
    }
    
    while(data.length>300*1024) {//0.25M-0.5M
