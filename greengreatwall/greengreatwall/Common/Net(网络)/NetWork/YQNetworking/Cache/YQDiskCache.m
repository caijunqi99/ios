...
+ (void)writeData:(id)data
    ...
    if (error) {
        //GPDebugLog(@"createDirectory error is %@",error.localizedDescription);
        return;
    }
...
+ (void)clearDataIinDir:(NSString *)directory {
    if (directory) {
        if ([[NSFileManager defaultManager] fileExistsAtPath:directory isDirectory:nil]) {
            ...
            if (error) {
                //GPDebugLog(@"清理缓存是出现错误：%@",error.localizedDescription);
            }
        }
    }
}
...
+ (void)deleteCache:(NSString *)fileUrl {
    if (fileUrl) {
        if ([[NSFileManager defaultManager] fileExistsAtPath:fileUrl]) {
            ...
            if (error) {
                //GPDebugLog(@"删除文件出现错误出现错误：%@",error.localizedDescription);
            }
        }else {
            //GPDebugLog(@"不存在文件");
        }
