//
//  GPWKWebViewWithHeaderAndFooter.m
//  greengreatwall
//
...
#import "GPWKWebViewWithHeaderAndFooter.h"

#import "SDPhotoBrowser.h"
@interface GPWKWebViewWithHeaderAndFooter()<WKNavigationDelegate,UIGestureRecognizerDelegate,SDPhotoBrowserDelegate>
//图片链接数组
...
@implementation GPWKWebViewWithHeaderAndFooter
...
-(void)setWkWebview:(WKWebView *)wkWebview{
    ...
    //    //GPDebugLog(@"wkWebview：%@",self.wkWebview);
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    ...
    [webView evaluateJavaScript:@"document.body.offsetHeight;" completionHandler:^(id Result, NSError * error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //            //GPDebugLog(@"高度%f",documentHeight);
            //            [DWBToast showCenterWithText:[NSString stringWithFormat:@"%f",documentHeight]];
            if (self.htmlString.length>0) {
                //GPDebugLog(@"self.headerView.height:%f",self.headerView.height);
                //啥也不干
            }else{
                documentHeight = 0.0;
            }
            
            self->_footView.frame = CGRectMake(0, documentHeight-self.footView.frame.size.height, GPScreenWidth, self.footView.frame.size.height);
            //加载完设置好坐标打开
            self->_footView.hidden = NO;
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)sender{
    ...
    [self evaluateJavaScript:imgJS completionHandler:^(id _Nullable imgUrl, NSError * _Nullable error) {
        if (imgUrl) {
            ...
            //保存图片
            [HPAlertTools showAlertWith:[self belongViewController] title:nil message:@"保存图片" callbackBlock:^(NSInteger btnIndex) {
                //GPDebugLog(@"%ld",(long)btnIndex);
                if (btnIndex == 1) {
                    //保存
                    dispatch_async(dispatch_get_main_queue(), ^{
                        //保存图片到相册
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if(!error){
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ShowAutoHideMBProgressHUD(GPKeyWindow, @"保存成功");
        });
        
    }else{
        //延迟显示，否则会移除
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ShowAutoHideMBProgressHUD(GPKeyWindow, @"保存失败");
        });
...
-(void)getImageUrlByJS:(WKWebView *)wkWebView{
    ...
    [wkWebView evaluateJavaScript:jsString completionHandler:^(id Result, NSError * error) {
        ...
        //        //GPDebugLog(@"得到所有图片url：%@",self.mArrayUrl);
    ...
    [self evaluateJavaScript:imgJS completionHandler:^(id _Nullable imgUrl, NSError * _Nullable error) {
        if (imgUrl) {
            ...
            if (self.mArrayUrl.count>0) {
                ...
                self.photoBrowser.sourceImagesContainerView = view;
                self.photoBrowser.currentImageIndex = currentIndex;
                
                [self.photoBrowser show];
                
            }else{
                //如果加载完后拿不到所有图片数组，就查看当前点击的图片
                self.mArrayUrl = [NSMutableArray arrayWithArray:@[imgUrl]];
