...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:SRVideoPlayerItemStatusKeyPath]) {
        //GPDebugLog(@"SRVideoPlayerItemStatusKeyPath");
        switch (playerItem.status) {
            case AVPlayerStatusReadyToPlay:
            {
                //GPDebugLog(@"AVPlayerStatusReadyToPlay");
            ...
            case AVPlayerStatusFailed:
            {
                ...
                //GPDebugLog(@"AVPlayerStatusReadyToPlay");
                //GPDebugLog(@"player error: %@", _player.error);
                //GPDebugLog(@"playerItem error: %@", _playerItem.error);
                [self.activityIndicatorView stopAnimating];
            ...
            {
                //GPDebugLog(@"AVPlayerStatusUnknown");
                break;
            }
        }
    }
    ...
    if ([keyPath isEqualToString:SRVideoPlayerItemLoadedTimeRangesKeyPath]) {
        //GPDebugLog(@"SRVideoPlayerItemLoadedTimeRangesKeyPath");
...
- (void)videoTopBarDidClickDownloadBtn {
    ...
    [[SRVideoDownloader sharedDownloader] downloadVideoOfURL:_videoURL progress:^(CGFloat progress) {
        //GPDebugLog(@"progress: %.2f", progress);
    } completion:^(NSString *cacheVideoPath, NSError *error) {
        if (cacheVideoPath) {
            //GPDebugLog(@"cacheVideoPath: %@", cacheVideoPath);
        } else {
            //GPDebugLog(@"error: %@", error);
        }
    }];
}
