...
@interface EditDeliveryAddressViewController ()<UITextFieldDelegate,AreaSelectDelegate>
{
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self updateInterfaceByModel:_addressModel];
}
...
- (void)configInterface
{
    ...
    [self setRightNavButtonWithImage:nil Title:@"删除" Frame:CGRectMake(0, 0, 30, 30) Target:self action:@selector(rightClick)];//GetImage(@"垃圾桶黑色")
    [self settingNavTitle:@"编辑收货人"];
...
-(void)rightClick
{
    ...
    [HPAlertTools showAlertWith:self title:@"提示信息" message:@"确定删除此内容吗" callbackBlock:^(NSInteger btnIndex) {
        if (btnIndex == 1) {
            //GPDebugLog(@"确定");
            [self netRequestDel];
        }
...
-(void)updateInterfaceByModel:(DeliveryAddressModel *)addressModel
{
    [_textFieldTemp[0] setText:addressModel.address_realname];
    [_textFieldTemp[1] setText:addressModel.address_mob_phone];
    [_textFieldTemp[2] setText:addressModel.area_info];
    [_textFieldTemp[3] setText:addressModel.address_detail];
    
    
    
    _buttonSetDefault.selected = ([addressModel.address_is_default isEqualToString:@"1"])?YES:NO;
    if (_buttonSetDefault.selected) {
        [_buttonSetDefault setImage:GetImage(@"选中") forState:UIControlStateNormal];
    }
}
...
-(void)netRequest
{
    ...
    {
        stringCity_id = _addressModel.city_id;
        stringRegion_id = _addressModel.area_id;
    }
    
    
    
    [HPNetManager POSTWithUrlString:HostMemberaddressaddress_edit isNeedCache:NO parameters:[NSDictionary dictionaryWithObjectsAndKeys:[HPUserDefault objectForKey:@"token"],@"key",_textFieldTemp[0].text,@"true_name",_textFieldTemp[1].text,@"mob_phone",stringCity_id,@"city_id",stringRegion_id,@"area_id",_textFieldTemp[3].text,@"address",_textFieldTemp[2].text,@"area_info",_buttonSetDefault.selected?@"1":@"0",@"is_default",@"1",@"longitude",@"1",@"latitude",_addressModel.address_id,@"address_id", nil] successBlock:^(id response) {
        //GPDebugLog(@"response:%@",response);
        if ([response[@"code"] integerValue] == 200) {
            [HPAlertTools showAlertWith:self title:@"提示信息" message:@"修改成功" callbackBlock:^(NSInteger btnIndex) {
...
{
    HPWeak;
    [HPNetManager POSTWithUrlString:HostMemberaddressaddress_del isNeedCache:NO parameters:[NSDictionary dictionaryWithObjectsAndKeys:[HPUserDefault objectForKey:@"token"],@"key",_addressModel.address_id,@"address_id", nil] successBlock:^(id response) {
        //GPDebugLog(@"response:%@",response);
        if ([response[@"code"] integerValue] == 200) {
            [HPAlertTools showAlertWith:self title:@"提示信息" message:@"删除成功" callbackBlock:^(NSInteger btnIndex) {
                [weakSelf.navigationController popViewControllerAnimated:YES];
                HPNOTIF_POST(@"refreshAddressList", nil);
            } cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:@"确定",nil];
        }
