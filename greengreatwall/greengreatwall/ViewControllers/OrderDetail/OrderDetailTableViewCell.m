...
-(void)layoutSubviews
{
    ...
    [_labelCount setFrame:RectWithScale(CGRectMake(900, 180, 100, 40), GPCommonLayoutScaleSizeWidthIndex)];
}
...
- (void)setGoodsModel:(GoodsModel *)goodsModel {
    ...
    self.labelCount.text = [NSString stringWithFormat:@"数量:%@", goodsModel.goods_num];
    [self.imageViewLeft sd_setImageWithURL:URL(goodsModel.goods_image) placeholderImage:defaultImage];
    self.labelTitle.text = goodsModel.goods_name;
    self.labelPrice.text = [NSString stringWithFormat:@"%@元", goodsModel.goods_price];
