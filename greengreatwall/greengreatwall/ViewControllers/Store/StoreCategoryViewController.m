...
static NSString * const storeCategoryCellReuseID  = @"storeCategoryCell";
...
- (void)configInterface
{
    ...
    [_collectionView registerClass:[StoreCategoryCollectionViewCell class] forCellWithReuseIdentifier:storeCategoryCellReuseID];
...
-(void)netRequest
{
    [HPNetManager GETWithUrlString:HostStorelistgetStoreClassList isNeedCache:NO parameters:nil successBlock:^(id response) {
        ...
        if ([dic[@"code"] integerValue] == 200) {
            if ([NSArray arrayWithArray:dic[@"result"]].count) {
                self->_arrayDataSource = [NSMutableArray arrayWithArray:dic[@"result"]];
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (identifier == nil) {
        identifier = [NSString stringWithFormat:@"%@%@", storeCategoryCellReuseID, [NSString stringWithFormat:@"%@", indexPath]];
