//
//  FundpredepositViewController.m
//  greengreatwall
//
...
@interface FundpredepositViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UIView                  *_viewTemp;
    ...
    UILabel                 *_labelTemp[4];
    UIImageView             *_imageViewTemp[2];
...
@implementation FundpredepositViewController
...
- (void)configInterface
{
    ...
    _viewRecharge = [UIView initViewBackColor:rgb(246, 247, 248)];
    [_viewTemp addSubview:_viewRecharge];
    [_viewRecharge setFrame:RectWithScale(CGRectMake(0, 420, 1080, 150), GPCommonLayoutScaleSizeWidthIndex)];
    ...
    [_viewRecharge addGestureRecognizer:tap];
    [_viewRecharge setUserInteractionEnabled:YES];
    
    _imageViewTemp[0] = [[UIImageView alloc]initWithImage:GetImage(@"奖金")];//储值卡钱包
    ...
    _imageViewTemp[1] = [[UIImageView alloc]initWithImage:GetImage(@"储值卡钱包")];
    [_viewRecharge addSubview:_imageViewTemp[1]];
    [_imageViewTemp[1] setFrame:RectWithScale(CGRectMake(450, 50, 50, 50),GPCommonLayoutScaleSizeWidthIndex)];
    
    
    
    NSArray *arrayLabelText = @[@"储值卡",@"¥ 50.00",@"收支记录",@"充值"];
    ...
    [_labelTemp[3] setFrame:RectWithScale(CGRectMake(540, 50, 400, 50), GPCommonLayoutScaleSizeWidthIndex)];
    [_viewRecharge addSubview:_labelTemp[3]];
    [_labelTemp[3] setTextColor:[UIColor blackColor]];
    _labelTemp[3].textAlignment = NSTextAlignmentLeft;
    
    
    _tableViewTemp = [[UITableView alloc]initWithFrame:CGRectMake(0, _viewInfo.bottom + kStatusBarHeight, GPScreenWidth, GPScreenHeight - kStatusBarHeight - _viewInfo.bottom) style:UITableViewStylePlain];
...
-(void)netRequest
{
    ...
    [HPNetManager POSTWithUrlString:Hostmembermy_asset isNeedCache:NO parameters:[NSDictionary dictionaryWithObjectsAndKeys:[HPUserDefault objectForKey:@"token"],@"key",@"predepoit",@"fields", nil] successBlock:^(id response) {
...
-(void)updateInterfaceWithDic:(NSDictionary*)dic
{
    ...
    NSString *stringavailable = [NSString stringWithFormat:@"¥ %@",datainfo[@"predepoit"]];
...
- (void)headerRereshing
{
    ...
    [HPNetManager POSTWithUrlString:Hostmemberfundpredepositlog isNeedCache:NO parameters:[NSDictionary dictionaryWithObjectsAndKeys:[HPUserDefault objectForKey:@"token"],@"key",_strParameter,@"page",@"10",@"pagesize", nil] successBlock:^(id response) {
        //GPDebugLog(@"response:%@",response);
...
{
    FundpredepositTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ReuseIdentify];
    if(!cell){
        cell = [[FundpredepositTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ReuseIdentify];
    }
